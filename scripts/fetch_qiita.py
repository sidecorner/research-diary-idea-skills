#!/usr/bin/env python3
"""
fetch_qiita.py - Fetch diary/journaling/habit-related articles from Qiita via Qiita API v2.
...
# Qiita search queries for diary/journaling topics (Japanese-primary)
# Queries are intentionally specific to diary/journaling to minimize Claude Code / AI tool noise
DIARY_QUERIES = [
    "日記 アプリ",
    "日記 個人開発",
    "ジャーナリング アプリ",
...
def fetch_qiita_articles(query: str, start_dt: datetime, end_dt: datetime,
    ...
    headers = {
        "User-Agent": "DiaryIdeaResearch/1.0",
...
def normalize_article(article: dict, query: str) -> dict:
    ...
    return {
        ...
        "tags": tags,
        "matched_query": query,
        # Higher weight on stocks (saves) as quality signal on Qiita
        "engagement_score": (
...
def main():
    parser = argparse.ArgumentParser(description="Fetch diary/journaling-related Qiita articles for idea research")
